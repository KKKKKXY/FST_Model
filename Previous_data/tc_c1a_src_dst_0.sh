...
create () {
  ...
  tc filter add dev $IF parent 1:0 prior 1 protocol ip u32 match ip dst $DST_Atomix2 classid 1:1
  # tc filter add dev $IF parent 1:0 prior 1 protocol ip u32 match ip src $DST_ONOS1 classid 1:1
  ...
  tc qdisc add dev $IF parent 1:1 handle 2:0 netem delay $atomix2_delay
